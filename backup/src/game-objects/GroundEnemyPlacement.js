...
tickAttemptAiMove() {
    ...
    const distanceToHero = this.calculateDistanceToHero();
    if (distanceToHero <= 50) {
      // Stop movement if hero is within 50 pixels
      return;
    }

    // Resume movement if hero is further than 50 pixels away
    this.internalMoveRequested(this.movingPixelDirection);
}

calculateDistanceToHero() {
    const [myX, myY] = this.displayXY();
    const [heroX, heroY] = this.level.heroRef.displayXY();
    const distance = Math.sqrt((myX - heroX) ** 2 + (myY - heroY) ** 2);
    return distance;
}
